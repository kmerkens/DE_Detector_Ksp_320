...
f1 = 'Palmyra05\Palmyra05_disk';     %input folder name 
...
for di = 1:12%size(inDir,1)
    ...
    for a=1:size(matNames,1)
        ...
        %eliminate all clicks with peak frequency below 20 kHz and above 55
        %kHz
        lowPeak=find(peakFr<20 | peakFr>55);
        F0(lowPeak)=[];
        dur(lowPeak)=[];
        nSamples(lowPeak)=[];
        peakFr(lowPeak)=[];
        ...
        %eliminate all clicks with center frequency below 25kHz and above
        %45kHz
        lowF0=find(F0<25 | F0>45);
        F0(lowF0)=[];
        dur(lowF0)=[];
        nSamples(lowF0)=[];
        peakFr(lowF0)=[];
        ...
        %eliminate all clicks with duration <0.25 ms
        shortDur=find(dur<0.25);
        F0(shortDur)=[];
        dur(shortDur)=[];
        nSamples(shortDur)=[];
        peakFr(shortDur)=[];
        ...
        %remove short duration samples (nSamples, -8dB limit) n<40 and n>80
        lowDur=find(nSamples<40 | nSamples>80);
        F0(lowDur)=[];
        dur(lowDur)=[];
        nSamples(lowDur)=[];
        peakFr(lowDur)=[];
...
save(newMatFile,'peakFrMTotal','F0MTotal','durMTotal','slopeMTotal','segTotal',...
    'nSamplesMTotal','rawDurTotal','rawStartTotal','diskNo');
