...
for n=1:length(speciesname);
    ...
    if ischar(matdatestr);                          % if logfile has only start dates
    ...
    spdatevecs{n} = datevec(eval([vnum{n}]));
        for k=1:size(spdatenums{n},1);
            ...
            Z_colstart = ((spdatevecs{n}(k,4))*60 + spdatevecs{n}(k,5));
            Z{n}(Z_rowstart, Z_colstart+1)=1; 
        end
        
    
    
    elseif iscell(matdatestr);                  %if logfile has start & end times
    ...
    %rowfind{n}=zeros(length(spdatevecs),1);
        for k=1:size(spdatenums{n},1);
            ...
            if isnan(spdatevecs{n}{rar});
                Z_colend=Z_colstart;
            else Z_colend=((spdatevecs{n}{rar}(4))*60 + spdatevecs{n}{rar}(5));
            end
    ...
    for b=0:round(length(days_of_data)/7)-1;     
        seventh_days(b+1,:)=datestr(days_of_data(b*7+1));
    end

    seventh_indices=[1:7:round(length(days_of_data)/7)*7];
    emptychars=blanks(size(datestr(days_of_data),2));
    
    %create empty character array before filling in every 7th datestr
    for j=1:length(days_of_data);
        day_labels(j,:)=emptychars;
    end
    day_labels(seventh_indices,:)=seventh_days;
    
    filepicked=txt{2,1};        %read in first input file name from log for site/deployment
                                %assume all picks within this logsheet
                                %came from same site & deployment
                             
    oyvey=strfind(filepicked,'_');  %let's hope site/deploymt comes before the first underscore
    deploymt=filepicked(1:oyvey(1)-1);    
 
    figure(n);
    set(gcf, 'units', 'inches',...
        'position', [3 2 6 4.5]);  %set figure size to print 2 to a page
    colormap([1 1 1; 0 0 0]);      %no detections=white, detection=black
    imagesc(Z{n});                 %plot presence-absence matrix
    h(n)=gca;                      %get current axes to set properties
    set(h(n),'Title', text('String',[deploymt, ' ', speciesname{n}]),...
