...
for n=1:length(speciesname);
    ...
    imagesc(Z{n});
    h(n)=gca;
    set(h(n), 'Title',text('String',speciesname{n},'FontWeight','bold'), ...
    'XTick',[min(get(h(n),'XLim')):60:max(get(h(n),'XLim'))-1], ...
    'XTickLabel', num2str([0:1:23]'), ...
    'YTick',[min(get(h(n),'YLim')):2:max(get(h(n),'YLim'))-1], ...
    'YTickLabel',datestr(days_of_data(1:2:length(days_of_data))), ...
    'XLabel',text('String','hour'), ...
    'XGrid','on', 'YGrid', 'on');
%     saveas(figure(n),[infile(1:8) '_' speciesname{n} '.fig'])
%     saveas(figure(n),[infile(1:8) '_' speciesname{n} '.jpg'])
%     saveas(figure(n),[infile(1:8) '_' speciesname{n} '.pdf'])
end
